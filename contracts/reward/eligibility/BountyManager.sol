...
contract BountyManager is Initializable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function quote(
    address _user
  ) public view whenNotPaused returns (uint256 bounty, uint256 actionType) {
  ...
  function executeBounty(
    ...
    (incentivizer, totalBounty, issueBaseBounty, actionType) = getBestBounty(
      _user,
      _execute,
      _actionType
    );
  ...
  function _canBountyHunt(address _user) internal view returns (bool) {
    ...
    bool isEmissionsEligible = IEligibilityDataProvider(eligibilityDataProvider)
  ...
  function getBestBounty(
    ...
    internal
    returns (address incentivizer, uint256 totalBounty, bool issueBaseBounty, uint256 actionType)
  {
