...
contract Leverager is Ownable {
  ...
  IViniumOracle public aaveOracle;
  ...
  function wethToZapEstimation(
    ...
    if (locked >= required) {
      ...
      uint8 priceDecimal = IChainlinkAggregator(aaveOracle.getSourceOfAsset(address(weth)))
        .decimals();
      uint256 wethAmount = deltaUsdValue.mul(10 ** 18).mul(10 ** priceDecimal).div(wethPrice).div(
        10 ** 8
      );
