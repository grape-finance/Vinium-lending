...
import '../../misc/interfaces/IViniumOracle.sol';
...
contract Compounder is OwnableUpgradeable, PausableUpgradeable {
  ...
  function _claimAndSwapToBase(address _user) internal returns (uint256) {
    ...
    ILendingPool lendingPool = ILendingPool(
      ILendingPoolAddressesProvider(addressProvider).getLendingPool()
    );
    ...
    for (uint256 i = 0; i < rewardBaseTokens.length; i++) {
      ...
      address underlying = IViToken(rewardBaseTokens[i]).UNDERLYING_ASSET_ADDRESS();
  ...
  function claimCompound(address _user, bool _execute) public returns (uint256 fee) {
    ...
    require(
      (PERCENT_DIVISOR * actualWethAfterSwap) / noSlippagePendingEth >= slippageLimit,
      'too much slippage'
    );
  ...
  function viewPendingRewards(
    address _user
  ) public view returns (address[] memory tokens, uint256[] memory amts) {
    IFeeDistribution.RewardData[] memory pending = IMultiFeeDistribution(multiFeeDistribution)
    ...
    for (uint256 i = 0; i < pending.length; i++) {
      if (pending[i].token != address(rdntToken)) {
        tokens[index] = IViToken(pending[i].token).UNDERLYING_ASSET_ADDRESS();
  ...
  function _estimateTokensOut(
    address _in,
    address _out,
    uint256 _amtIn
  ) internal view returns (uint256 tokensOut) {
    IViniumOracle oracle = IViniumOracle(
      ILendingPoolAddressesProvider(addressProvider).getPriceOracle()
    );
