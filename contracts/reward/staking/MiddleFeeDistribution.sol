...
contract MiddleFeeDistribution is IMiddleFeeDistribution, Initializable, OwnableUpgradeable {
  ...
  function emitNewTransferAdded(address asset, uint256 lpReward) internal {
    if (asset != address(rdntToken)) {
      address underlying = IViToken(asset).UNDERLYING_ASSET_ADDRESS();
      uint256 assetPrice = IViniumOracle(_aaveOracle).getAssetPrice(underlying);
      address sourceOfAsset = IViniumOracle(_aaveOracle).getSourceOfAsset(underlying);
