...
contract ParaSwapLiquiditySwapAdapter is BaseParaSwapSellAdapter, ReentrancyGuard {
   ...
   * The user should give this contract allowance to pull the ViTokens in order to withdraw the underlying asset and repay the flash loan.
   ...
   * The user should give this contract allowance to pull the ViTokens in order to withdraw the underlying asset and perform the swap.
  ...
  function _swapLiquidity(
    ...
    if (swapAllBalanceOffset != 0) {
      ...
      require(balance >= amountToSwap.add(premium), 'INSUFFICIENT_VITOKEN_BALANCE');
    }
